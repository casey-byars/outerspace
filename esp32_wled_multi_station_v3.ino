/*
 * ESP32 WLED Multi-Station Touch Controller v3.0
 * 
 * Three-puzzle cooperative system:
...
const char* tabletIP3 = "192.168.1.203";    // iPad/tablet IP for station 3
...
void setup() {
  Serial.begin(115200);
  Serial.println("ESP32 Multi-Station Touch Controller v3.0 Starting...");
...
<body>
    <div class="container">
        ...
        <div class="status">
            ...
            <p>4. All stations must complete their videos to advance</p>
        </div>
    </div>
    ...
    <script>
        ...
        setInterval(function() {
            fetch('/status')
                ...
                .then(data => {
                    if (data.videoStarted && !videoStarted && video.paused) {
                        video.play();
                    }
                });
        }, 2000);
  ...
  for (int i = 1; i <= 3; i++) {
    if (i != STATION_ID) {
      if (currentTime - stationStates[i].timestamp > TIMEOUT_THRESHOLD) {
        if (stationStates[i].touchActive || stationStates[i].mazeComplete || stationStates[i].videoStarted) {
          stationStates[i].touchActive = false;
          stationStates[i].mazeComplete = false;
...
void handlePuzzle3Logic() {
  bool allStationsVideoComplete = true;
  
  for (int i = 1; i <= 3; i++) {
    if (!stationStates[i].videoComplete) {
      allStationsVideoComplete = false;
      break;
    }
  }
  
  if (allStationsVideoComplete && currentPuzzleState == PUZZLE2_COMPLETE) {
    currentPuzzleState = PUZZLE3_ACTIVE;
    Serial.println("=== PUZZLE 3 ACTIVE ===");
    Serial.println("All videos started!");
    
  } else if (allStationsVideoComplete && currentPuzzleState == PUZZLE3_ACTIVE) {
    currentPuzzleState = PUZZLE3_COMPLETE;
    
    Serial.println("=== PUZZLE 3 SOLVED! ===");
    Serial.println("All videos completed successfully!");
...
void triggerTabletVideo() {
  String tabletIP;
  switch (STATION_ID) {
    case 1: tabletIP = tabletIP1; break;
    case 2: tabletIP = tabletIP2; break;
    case 3: tabletIP = tabletIP3; break;
    default: return;
  }
...
void printInstructions() {
  ...
  Serial.println("- All 3 videos must be completed");
